...
const pendingTickets = computed(() => tickets.value.filter(ticket => ticket.state !== 'Resolved'));
      ...
      <tbody>
        ...
        <tr v-else v-for="ticket in paginatedPendingTickets" :key="ticket.id">
          ...
          <td class="d-flex justify-content-center align-items-center">
            <div class="btn-group">
              <a class="btn btn-app bg-success" :data-target="'#updateTicket' + ticket.id" data-toggle="modal"
                style="margin-left: 10px;">
                <i class="fas fa-clipboard-check"></i> Update
              </a>
              <UpdateTickets :ticket="ticket" :id="String(ticket.id)" />

              <button class="btn btn-app bg-danger" @click="destroyTicket(ticket.id)" style="margin-left: 10px;">
